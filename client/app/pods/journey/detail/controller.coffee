...
Controller = Ember.Controller.extend
  ...
  actions:
    onEntry: `function(){
        var datafile = this.get('datafile');
        //alert("jQuery version: " + jQuery.fn.jquery + " datafile: " + datafile);
        jQuery.ajax({
            url: window.ENV['metrocarServer'] + '/api/v1/datafile/' + datafile + '/',
            success: function(data, status, xhr) {
                window.loadAndDrawJourney(data, $(".journey-detail"));
            }
        });
    }`
