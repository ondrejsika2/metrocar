...
class Serializer(base.Serializer):
    ...
    def handle_related_field(self, obj, field):
        """
        Handles related field to the model.
        """
        if hasattr(obj, field.get_accessor_name()):
            fname = field.get_accessor_name()
            ...
            if related_obj is not None:
                if fname in self.relations:
                    ...
                    if isinstance(self.relations, dict):
                        if isinstance(self.relations[fname], dict):
                            options = self.relations[fname]
                    self._fields[fname] = serializer.serialize([related_obj],
                        **options)[0]
